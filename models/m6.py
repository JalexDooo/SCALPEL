...
class m6(nn.Module):
    ...
    def forward(self, x, device='cuda:2'):
        batch = x.shape[0]
        g_seq = x[:, 0:120].view(batch, 30, 4)
        t_seq = x[:, 120:240].view(batch, 30, 4)
        g_fold = x[:, 240:330].view(batch, 30, 3)
        g_mfe = x[:, 330].view(batch, 1)
        h_mfe = x[:, 331].view(batch, 1)
        icshape = x[:, 332:362].view(batch, 30)
        binding_p = x[:, 362:534].view(batch, 172)
        rpkm = x[:, 534].view(batch, 1)
        st_relate_len = x[:, 535].view(batch, 1)
        ed_relate_len = x[:, 536].view(batch, 1)
        utr5_rate = x[:, 537].view(batch, 1)
        cds_rate = x[:, 538].view(batch, 1)
        utr3_rate = x[:, 539].view(batch, 1)
        bert_embedding = x[:, 540:].view(batch, 28, 768)
        ...
        if self.bert:
            ...
            features = torch.cat([features, bert_feat], dim=1)
        ...
        if self.fold:
            ...
            features = torch.cat([features, fold1d], dim=1)

        if self.mfe1:
            features = torch.cat([features, g_mfe], dim=1)
        if self.mfe2:
            features = torch.cat([features, h_mfe], dim=1)
        if self.icshape:
            features = torch.cat([features, icshape], dim=1)
        if self.binding:
            features = torch.cat([features, binding_p], dim=1)
        if self.rpkm:
            features = torch.cat([features, rpkm], dim=1)
        if self.rt1 | self.rt2:
            features = torch.cat([features, st_relate_len], dim=1)
            features = torch.cat([features, ed_relate_len], dim=1)
        if self.utr1 | self.utr2 | self.utr3:
            features = torch.cat([features, utr5_rate], dim=1)
            features = torch.cat([features, cds_rate], dim=1)
            features = torch.cat([features, utr3_rate], dim=1)
