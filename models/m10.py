...
class m10_m1(nn.Module):
    '''
    Add bert embedding.
    '''
    def __init__(self, des='seq'):
        super(m9_m1, self).__init__()
        ...
        if 'bert' in des:
            ...
            self.cnn_self_attn = nn.MultiheadAttention(embed_dim=self.d_model, num_heads=4, batch_first=True)
            self.norm_self_attn_cnn = nn.LayerNorm(self.d_model)
            self.dropout_self_attn_cnn = nn.Dropout(0.1)
            ...
            self.cnn_k_proj2 = nn.Linear(self.d_model, self.d_model)
            self.cnn_v_proj2 = nn.Linear(self.d_model, self.d_model)
        ...
        if 'icshape' in des:
            ...
            self.icshape_ext = nn.Sequential(
        ...
        self.seq1 = nn.Sequential(
            nn.Conv2d(2, 64, kernel_size=4, stride=1, padding='same'),
            nn.BatchNorm2d(64),
            nn.ReLU(),
            ResBlock2d(64),
            ResBlock2d(64),
        ...
        baseline_phys_dim = 0
        if self.fold: baseline_phys_dim += 180
        if self.mfe1: baseline_phys_dim += 1
        if self.mfe2: baseline_phys_dim += 1
        if self.icshape:

            baseline_phys_dim += 30
        if self.binding: baseline_phys_dim += 172
        if self.rpkm: baseline_phys_dim += 1
        if self.rt1 | self.rt2: baseline_phys_dim += 2
        if self.utr1 | self.utr2 | self.utr3: baseline_phys_dim += 3

        self.phys_proj = nn.Linear(baseline_phys_dim, self.d_model) if baseline_phys_dim > 0 else None
    ...
    def forward(self, x, device='cuda:3'):
        ...
        if self.bert:
            ...
            if self.fold:
                ...
                fold1 = self.conv2(fold1)
                fold1 = self.maxpool(fold1)
                fold1_flat = self.flat(fold1)
                fold1_d = self.dropout2(fold1_flat)
                phys_list.append(fold1_d)
            if self.mfe1: phys_list.append(g_mfe)
            if self.mfe2: phys_list.append(h_mfe)
            if self.icshape:
                icshape = icshape.unsqueeze(1)
                icshape_conv = self.icshape_ext(icshape)
                icshape_flat = self.flat(icshape_conv)
                phys_list.append(icshape_flat)
            if self.binding: phys_list.append(binding_p)
            if self.rpkm: phys_list.append(rpkm)
            if self.rt1 | self.rt2:
                phys_list.append(st_relate_len)
                phys_list.append(ed_relate_len)
            if self.utr1 | self.utr2 | self.utr3:
                phys_list.append(utr5_rate)
                phys_list.append(cds_rate)
                phys_list.append(utr3_rate)

            physics_raw = torch.cat(phys_list, dim=1) if len(phys_list) > 0 else None

            if physics_raw is not None and self.phys_proj is not None:

                physics_state = self.phys_proj(physics_raw)

                physics_token = physics_state.unsqueeze(1)
            else:
                physics_token = None

            padding_mask = (attention_mask == 0)

            cnn_q1_raw = self.cnn_q_proj1(cnn_seq)

            if physics_token is not None:
                cnn_q1 = torch.cat([physics_token, cnn_q1_raw], dim=1)
            else:
                cnn_q1 = cnn_q1_raw

            self_attn_out_cnn, _ = self.cnn_self_attn(query=cnn_q1, key=cnn_q1, value=cnn_q1)
            self_attn_out_cnn = self.dropout_self_attn_cnn(self_attn_out_cnn)
            cnn_q1 = self.norm_self_attn_cnn(cnn_q1 + self_attn_out_cnn)
            ...
            cnn_k2 = self.cnn_k_proj2(cnn_q1)
            cnn_v2 = self.cnn_v_proj2(cnn_q1)
        ...
        if not self.bert:
            ...
            if self.mfe1: features = torch.concat([features, g_mfe], dim=1)
            if self.mfe2: features = torch.concat([features, h_mfe], dim=1)
            if self.icshape:
                icshape = icshape.unsqueeze(1)
                icshape_conv = self.icshape_ext(icshape)
                icshape_flat = self.flat(icshape_conv)
                features = torch.concat([features, icshape_flat], dim=1)
            if self.binding: features = torch.concat([features, binding_p], dim=1)
            if self.rpkm: features = torch.concat([features, rpkm], dim=1)
            if self.rt1 | self.rt2:
                features = torch.concat([features, st_relate_len], dim=1)
                features = torch.concat([features, ed_relate_len], dim=1)
            if self.utr1 | self.utr2 | self.utr3:
            ...
            if physics_raw is not None:
                features = torch.cat([features, physics_raw], dim=1)
